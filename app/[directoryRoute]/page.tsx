...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (parsed.isHub) {
    return {
      ...
      description: `Find ${parsed.category.displayName.toLowerCase()} across all 36 Nigerian states and the FCT. Browse local artists offering ${parsed.category.description}.`,
    };
  }
  ...
  return {
    ...
    description: artists.length
      ? `Browse ${artists.length} approved ${parsed.category.displayName.toLowerCase()} serving ${state.name}, Nigeria. Compare portfolios and contact local artists directly.`
...
export default async function DirectoryRoutePage({ params }: Props) {
  ...
  if (parsed.isHub) {
    ...
    return <>
      <section className="page-hero directory-page-hero"><div className="shell directory-hero-grid"><div className="directory-hero-copy">
        ...
        <p>Find professional Nigerian creatives specializing in {category.description}. Choose a state to browse local profiles, portfolios and contact information.</p>
  ...
  const jsonLd = artists.map((artist) => ({
  ...
  return <>
    ...
    <section className="page-hero directory-page-hero"><div className="shell directory-hero-grid"><div className="directory-hero-copy">
      ...
      <p>Find approved {category.displayName.toLowerCase()} serving {state!.name}. Compare portfolios and contact an artist directly for {category.description}.</p>
    ...
    <section className="section"><div className="shell">
      <div className="section-heading"><p className="eyebrow">Approved artist listings</p><h2>Featured {category.singular} Serving {state!.name}</h2></div>
      ...
      <div className="section-heading" style={{ marginTop: 90 }}><p className="eyebrow">Commission with confidence</p><h2>How to Hire a {category.singular} in {state!.name}</h2><p>Compare portfolios, experience and pricing before hiring. Agree in writing on the creative brief, materials, total cost, delivery date and revision process before the artist begins.</p></div>
